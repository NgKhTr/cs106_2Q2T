...
def parse_args():
    ...
    # parser.add_argument("--memo", type=str, default='3_3_Colight')
    # parser.add_argument("--memo", type=str, default='6_6_Colight')
    # parser.add_argument("--memo", type=str, default='Hangzhou_Colight')
    parser.add_argument("--memo", type=str, default='Jinan_Colight')

    # parser.add_argument("-b", action="store_true",default=True, help="run baseline analysis") ##
    # parser.add_argument("--memo", type=str, default='3_3_Fixedtime')
    # parser.add_argument("--memo", type=str, default='6_6_Fixedtime')
    # parser.add_argument("--memo", type=str, default='Hangzhou_Fixedtime')
    # parser.add_argument("--memo", type=str, default='Jinan_Fixedtime')

    # parser.add_argument("--memo", type=str, default='3_3_MaxPressure')
    # parser.add_argument("--memo", type=str, default='6_6_MaxPressure')
    # parser.add_argument("--memo", type=str, default='Hangzhou_MaxPressure')
    # parser.add_argument("--memo", type=str, default='Jinan_MaxPressure')
...
def drawConvergenceChart(duration_list, folderResultPath):
    sns.set_theme()
    X = list(range(duration_list.shape[0]))
    sns.lineplot(x=X, y=duration_list)
    plt.title(MAP_NAME)
    plt.xlabel("Round")
    plt.ylabel("Duration")
    # Hiển thị đồ thị
    plt.savefig(folderResultPath)
    plt.close()
...
def get_metrics(duration_list, queue_length_list, min_duration, min_duration_id, min_queue_length, min_queue_length_id,
    ...
    validation_duration_length = 10 # Lấy 10 cái duration ở cuối 
    minimum_round = 50 if num_rounds > 50 else 0 # bỏ qua 50 cái đầu
    ...
    # Trung bình của 10 cái duration ở cuối
    ...
    if nan_count == 0: # kiểm kiểm tra gì đó thôi
            ...
            for level in [1.2, 1.1]:
                if max(duration_list[j:]) <= level * final_duration:
                    if convergence[level] > j:
                        convergence[level] = j
        # Nếu giá trị tại chổ nào mà: final_duration < 1.2 * giá trị => Hội tụ 1.2
        # Nếu giá trị tại chổ nào mà: final_duration < 1.1 * giá trị => Hội tụ 1.1
        conv_12 = convergence[1.2] # chứa round được xem là hội tụ
        conv_11 = convergence[1.1] # chứa round được xem là hội tụ
    else:
        conv_12, conv_11 = 0, 0


    drawConvergenceChart(duration_list, save_path + "/" + traffic_name + "-" + mode_name + ".png")
    '''
    # simple plot for each training instance
    f, ax = plt.subplots(1, 1, figsize=(12, 9), dpi=100)

    # vẽ đường hội tụ: màu đen
    ax.plot(duration_list, linewidth=2, color='k')

    # vẽ đường ngang
    # Điểm đầu: 
    #   (0, len(duration_list))
    # Điểm cuối: (final_duration, final_duration)
    ax.plot([0, len(duration_list)], [final_duration, final_duration], linewidth=2, color="g")


    # vẽ mấy cái đường dọc: đánh dấu vị trí hội tụ
    ax.plot([conv_11, conv_11], [duration_list[conv_11] - 100, duration_list[conv_11] + 100], linewidth=1, color="b")
    ax.plot([conv_12, conv_12], [duration_list[conv_12] - 100, duration_list[conv_12] + 100], linewidth=1, color="b")
    # điểm đầu:
    #   (conv_12, duration_list[conv_12])
    # điểm cuối:
    #   (conv_12, duration_list[conv_12] * 3)
    # tại sao lại có tới 2 đường ???

    # đường này đéo thấy
    ax.plot([0, len(duration_list)], [min_duration, min_duration], linewidth=1, color="r", linestyle='dashed')

    ax.plot([min_duration_id, min_duration_id], [min_duration - 100, min_duration + 100], linewidth=1, color="r")

    ax.set_title(traffic_name + "-" + str(final_duration))
    
    path = save_path + "/" + traffic_name + "-" + mode_name + ".png"
    
    plt.savefig(save_path + "/" + traffic_name + "-" + mode_name + ".png") # vẽ đồ thị hội tụ
    plt.close()
    '''
...
def plot_segment_duration(round_summary, path, mode_name):
    # hàm này nó sẽ vẽ duration trung bình trong 1 segment cố định ở mỗi round
    # so sánh duration trong cùng 1 segment cố định ở mỗi round
    ...
    for key in round_summary.keys():
        if "duration" in key:
            ...
            ax.set_title(key)
            plt.savefig(save_path + "/" + key + "-" + mode_name + ".png") # vẽ trong segment: hiện tại đang lỗi ko hiển thị qq gì hết
            plt.close()
...
def summary_detail_test(memo, total_summary):
    ...
    for traffic_file in os.listdir(records_dir):
        ...
        time_interval = 120 # sao chổ này nó lại cố định ??? # này chỉ dùng cho cái khỉ dưới kia thôi
        ...
        for round in round_files:
            ...
            for inter_index in range(num_intersection):
                ...
                try:
                    ...
                    ave_duration = df_vehicle_inter['duration'].mean(skipna=True) # cái này đéo có xài
            ...
            duration_each_round_list.append(ave_duration) # trung bình duration của 1 round
            ...
            if min_duration > ave_duration:
                min_duration = ave_duration
                min_duration_ind = int(round[6:]) # round có định dạng là: round_[số]
        ...
        result.to_csv(os.path.join(result_dir, f"{memo}.csv"))
...
def my_summary_detail_test(memo):
    records_dir = os.path.join("records", memo)
    ...
    for traffic_file in os.listdir(records_dir):
        ...
        for inter_index in range(num_intersection):
            # try:
            f = open(os.path.join(records_dir, traffic_file, "inter_{0}.pkl".format(inter_index)), "rb")
            samples = pkl.load(f)
            queue_length_each_inter_each_round = 0
            for sample in samples:
                queue_length_each_inter_each_round += sum(sample['state']['lane_num_vehicle_been_stopped_thres1'])
            ...
            # summary items (duration) from csv
            df_vehicle_inter = pd.read_csv(os.path.join(records_dir, traffic_file, "vehicle_inter_{0}.csv".format(inter_index)),
                                                sep=',', header=0, dtype={0: str, 1: float, 2: float},
                                                names=["vehicle_id", "enter_time", "leave_time"])
            df_vehicle_inter['leave_time_origin'] = df_vehicle_inter['leave_time']
            df_vehicle_inter['leave_time'].fillna(run_counts,inplace=True)
            ...
            df_vehicle_all.append(df_vehicle_inter)
            queue_length_list.append(queue_length_each_inter_each_round)

            # except:
            #     queue_length_list.append(NAN_LABEL)

        # df_vehicle_all = pd.concat(df_vehicle_all)
        print(len(df_vehicle_all))
        df_vehicle_all = pd.concat(df_vehicle_all)
        vehicle_duration = df_vehicle_all.groupby(by=['vehicle_id'])['duration'].sum()
        ave_duration = vehicle_duration.mean()
        ave_queue_length = np.mean(queue_length_list)
        ...
        if not os.path.exists(result_dir):
            os.makedirs(result_dir)
        _res = {
            "duration": [ave_duration],
            "queue_length": [ave_queue_length],
            "vehicle_in": num_of_vehicle_in,
            "vehicle_out": num_of_vehicle_out
        }
        result = pd.DataFrame(_res)
        ...
        if os.path.getsize(os.path.join(train_dir, "inter_0.pkl")) > 0:
            ...
            for f in list_f:
                ...
                if ANON_ENV:
                    '''
                    print('Print')
                    print(df_vehicle_inter_0['vehicle_id'])
                    temp = df_vehicle_inter_0['vehicle_id'].str
                    # flow_car = pd.DataFrame(df_vehicle_inter_0['vehicle_id'].str.split('_', -1).tolist(), columns=['flow','flow_id', 'car_id'])
                    # hình như ý đồ là:
                    # 1 loạt 
                    flow_car = pd.DataFrame(df_vehicle_inter_0['vehicle_id'].str.split('_').tolist(), columns=['flow','flow_id', 'car_id'])
                    print(flow_car.head())
                    '''
                    flow_car = pd.DataFrame(df_vehicle_inter_0['vehicle_id'].str.split('_').tolist(), columns=['flow','flow_id', 'car_id'])
                    flow_car = flow_car.drop('flow', axis=1)
                else:
...
if __name__ == "__main__":
    ...
    if args.b:
        ...
        my_summary_detail_test(args.memo)
    else:
